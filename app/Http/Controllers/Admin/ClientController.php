...
class ClientController extends Controller
{
    ...
    public function save(SaveClientRequest $request): RedirectResponse
    {
        ...
        try {
            ...
            session()->flash('success', __('client.success_created', ['name' => $client->getName()]));
        } catch (Exception $e) {
            session()->flash('error', __('client.flash_save_error', ['error' => $e->getMessage()]));
        }
    ...
    public function update(UpdateClientRequest $request, string $id): RedirectResponse
    {
        ...
        try {
            ...
            session()->flash('success', __('client.success_edited', ['name' => $client->getName()]));
        } catch (Exception $e) {
            session()->flash('error', __('client.flash_update_error', ['error' => $e->getMessage()]));
        }
    ...
    public function destroy(string $id): RedirectResponse
    {
        try {
            ...
            if ($client->projects()->exists()) {
                session()->flash('error', __('client.cant_delete', ['name' => $client->getName()]));

                return redirect()->route('admin.client.index');
            }

            $client->delete();
            session()->flash('success', __('client.success_deleted', ['name' => $client->getName()]));
        } catch (Exception $e) {
            session()->flash('error', __('client.flash_delete_error', ['error' => $e->getMessage()]));
        }
