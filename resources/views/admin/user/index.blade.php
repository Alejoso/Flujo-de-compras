          ...
          <tbody>
            @forelse ($viewData['users'] as $user)
              <tr class="um-row">
                ...
                <td data-label="{{ __('admin_user.th_name') }}"><span class="um-user-name">{{ $user->getName() }}</span>
                </td>
                ...
                <td data-label="{{ __('admin_user.th_cedula') }}">{{ $user->getIdNumber() }}</td>
                <td data-label="{{ __('admin_user.th_phone') }}">{{ $user->getPhoneNumber() }}</td>
                <td data-label="{{ __('admin_user.th_salary') }}">$ {{ number_format($user->getSalary(), 0, '', '.') }}
                </td>
                <td data-label="{{ __('admin_user.th_notifications') }}">
                  <span class="um-status um-status--{{ $user->getReceivesNotifications() ? '1' : '0' }}">
                    {{ $user->getReceivesNotifications() ? __('admin_user.yes') : __('admin_user.no') }}
                  </span>
                </td>
                <td data-label="{{ __('admin_user.th_actions') }}">
                  <div class="um-actions">
                    <a href="{{ route('admin.user.edit', $user->getId()) }}" class="um-btn-icon um-btn-icon--edit"
                      title="{{ __('admin_user.btn_save_changes') }}">
                      <i class="bi bi-pencil-fill"></i>
                    </a>
                    <form action="{{ route('admin.user.destroy', $user->getId()) }}" method="POST"
                      onsubmit="return confirm('{{ __('admin_user.confirm_delete') }}')">
                      @csrf
                      @method('DELETE')
                      <button type="submit" class="um-btn-icon um-btn-icon--delete"
                        title="{{ __('admin_user.th_actions') }}">
