...
@section('content')
  <div class="pj-wrapper" id="quotation-app" data-search-url="{{ route('admin.materials.search') }}"
    ...
    <div class="um-header">
      ...
      <a href="{{ route('admin.quotation.show', [$viewData['project']->getId(), $viewData['version']->getId()]) }}"
        class="um-btn-icon um-btn-icon--secondary px-3 py-2">
        <i class="bi bi-x-circle me-1"></i> {{ __('technician_quotation.btn_cancel') }}
      </a>
    </div>

    <form action="{{ route('admin.quotation.update', [$viewData['project']->getId(), $viewData['version']->getId()]) }}"
      method="POST">
      @csrf
      @method('PATCH')
